...
pub struct MapEncoder {
    ...
    fn encode(&self, payload: &str, _: &EncodeOptions) -> Result<String, Error> {
        shared::remap(payload, self.token2unicode)
    }
    fn decode(&self, payload: &str) -> Result<String, Error> {
        shared::remap(payload, self.unicode2token)
    }
    fn can_encode(&self, payload: &str) -> Result<(), Error> {
        shared::covered_by_alphabet(payload, self.token2unicode)
    }
}
...
pub struct TokenEncoder {
    ...
    fn encode(&self, payload: &str, options: &EncodeOptions) -> Result<String, Error> {
        match options.level {
            CompressionLevel::Fast => return strategies::first_match(&payload, self.token2unicode, self.token_max_chars as usize),
            CompressionLevel::Balanced => return strategies::longest_match(&payload, self.token2unicode, self.token_max_chars as usize),
        }
    }
    fn decode(&self, payload: &str) -> Result<String, Error> {
        shared::remap(payload, self.unicode2token)
